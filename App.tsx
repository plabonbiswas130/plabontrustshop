...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'main' | 'dropshipping' | 'warehouse' | 'admin' | 'admin-editor' | 'profile' | 'videos' | 'ecom-dashboard'>('main');
  ...
  if (!isLoggedIn) {
   ...
   if (currentView === 'account-access') {
     return <AccountAccess onBack={() => setCurrentView('main')} onAdminSuccess={() => setCurrentView('admin')} t={t} />;
   }
    ...
    if (config?.source_type === 'file' && config.files && config.files.length > 0 && audioRef.current) {
        ...
        const handleEnded = () => {
            if (config.files.length > 1) {
                setCurrentIndex((prev) => (prev + 1) % config.files.length);
            } else if (config.play_mode === 'loop') {
  ...
  return (
    <>
      <audio 
        ...
        style={{ display: 'none' }}
        src={(config?.source_type === 'file' && !config.files ? (config?.source_url || null) : null)}
      />
